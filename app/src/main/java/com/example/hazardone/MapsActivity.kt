...
class MapsActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        ViewCompat.setOnApplyWindowInsetsListener(binding.root) { _, insets ->
            ...
            val spacing = resources.getDimensionPixelSize(R.dimen.fab_spacing)

            binding.reportButton.updateLayoutParams<ConstraintLayout.LayoutParams> {
                rightMargin = spacing + systemBars.right
                bottomMargin = spacing + systemBars.bottom
            }
            binding.myLocationFab.updateLayoutParams<ConstraintLayout.LayoutParams> {
                rightMargin = spacing + systemBars.right
            }
            insets
        }
    ...
    private fun setupThemeSwitch(menu: Menu?) {
        val actionView = menu
            ?.findItem(R.id.action_theme_mode)
            ?.actionView
            ?: return

        val themeSwitch = actionView.findViewById<SwitchMaterial>(R.id.themeSwitch)
        val themeLabel = actionView.findViewById<TextView>(R.id.themeModeLabel)
        val isDarkMode = ThemePreference.isDarkMode(this)

        updateThemeSwitch(themeSwitch, themeLabel, isDarkMode)

        themeSwitch.setOnCheckedChangeListener { _, checked ->
            updateThemeSwitch(themeSwitch, themeLabel, checked)
            ThemePreference.setDarkMode(this, checked)
        }
    ...
    private fun updateThemeSwitch(
        themeSwitch: SwitchMaterial,
        themeLabel: TextView,
        isDarkMode: Boolean
    ) {
        themeSwitch.isChecked = isDarkMode
        themeLabel.text = getString(
            if (isDarkMode) {
                R.string.theme_dark
            } else {
